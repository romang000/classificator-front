...
import type { BreedGetByPropertyValueResponse, CatFeatures, RankRequest, RankResponse } from "../../../entities/breed/model/types"
...
export function ResultPage() {
  ...
  const breedNames = result?.breeds?.map((b: any) => b.name) || []
    ...
    <>
      ...
      <div className="appFrame">
        ...
        <div className="contentBox">
          ...
          {/* Объяснение */}
          <div style={{ marginBottom: 20 }}>
            <ExplanationCard text="Анализ проведен на основе выбранных характеристик. В развитии: подробные объяснения по каждой альтернативе." />
          </div>

          {/* Результат уточнения модели */}
          {modelResult && (
            <div style={{ marginBottom: 20 }}>
              <div className="innerPanel" style={{ background: 'linear-gradient(135deg, rgba(100, 150, 200, 0.08) 0%, rgba(100, 150, 200, 0.04) 100%)', borderColor: 'rgba(100, 150, 200, 0.3)' }}>
                <div className="innerPanelTitle" style={{ color: '#6496c8', fontSize: 20, marginBottom: 12 }}>
                  Уточнение нейросети
                </div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#2a2a2a', marginBottom: 12 }}>
                  {modelResult.selectedBreed}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ fontSize: 14, color: '#6e6a75', fontWeight: 500 }}>Уверенность:</div>
